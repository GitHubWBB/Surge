/**
 * ⚽世界杯·今日赛程
 * Surge type=generic | 显示今天+明天的比赛（纯赛程，不显示比分）
 */
var FLAGS = {
...
function fmtLine(item) {
  ...
  return icon+" "+item.time+" "+hF+hC+" vs "+aF+aC+" ["+m.g+"组]";
}

if (todayM.length===0 && tmrwM.length===0) {
  $done({title:"⚽世界杯·今日赛程", content:"今日和明日均无比赛\n\n小组赛 6/12-6/28\n决赛 7/20 03:00", icon:"soccerball", "icon-color":"#8E8E93"});
} else {
  var lines = [];
  if (todayM.length > 0) {
    lines.push("今日 "+todayM.length+" 场");
    for (var i=0;i<todayM.length;i++) lines.push(fmtLine(todayM[i]));
  }
  if (tmrwM.length > 0) {
    if (lines.length > 0) lines.push("");
    lines.push("明日 "+tmrwM.length+" 场");
    for (var i=0;i<tmrwM.length;i++) lines.push(fmtLine(tmrwM[i]));
  }
