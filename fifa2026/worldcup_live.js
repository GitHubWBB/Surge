...
var now = new Date();
var liveMatches = [], recentFinished = [], upcoming2h = [];
...
for (var i = 0; i < MATCHES.length; i++) {
  ...
  if (elapsed > -5 * 60000 && elapsed < 2 * 3600000) {
    ...
    upcoming2h.push({ m: m, time: hh + ":" + mm, ts: bj.getTime(), mins: Math.round(-elapsed / 60000) });
  }
}

if (apiKey) {
  var td = now.getFullYear() + "-" + String(now.getMonth()+1).padStart(2,"0") + "-" + String(now.getDate()).padStart(2,"0");
  $httpClient.get({ url: "https://api.football-data.org/v4/competitions/WC/matches?dateFrom=" + td + "&dateTo=" + td, headers: { "X-Auth-Token": apiKey } }, function(error, response, data) {
    var apiData = null;
    if (!error && data) { try { apiData = JSON.parse(data); } catch(e) {} }
    renderLive(liveMatches, recentFinished, upcoming2h, apiData);
  });
} else {
  renderLive(liveMatches, recentFinished, upcoming2h, null);
}

function getScore(m, apiData) {
  ...
  for (var i = 0; i < apiData.matches.length; i++) {
    ...
    if ((am.homeTeam.name === m.h || CN[am.homeTeam.name] === CN[m.h]) &&
      ...
      var hs = am.score.fullTime.home !== null ? am.score.fullTime.home : (am.score.halfTime.home !== null ? am.score.halfTime.home : "?");
      var as2 = am.score.fullTime.away !== null ? am.score.fullTime.away : (am.score.halfTime.away !== null ? am.score.halfTime.away : "?");
      return hs + "-" + as2 + (am.minute ? " " + am.minute + "'" : "");
    }
  }
  return "vs";
}

function renderLive(live, finished, upcoming, apiData) {
  var lines = [];
  ...
  if (live.length > 0) {
    ...
    for (var i = 0; i < live.length; i++) {
      var m = live[i].m;
      lines.push("  " + (FLAGS[m.h]||"🏳️") + (CN[m.h]||m.h) + " " + getScore(m, apiData) + " " + (FLAGS[m.a]||"🏳️") + (CN[m.a]||m.a) + " [" + m.g + "组]");
    }
    lines.push("");
  }
  ...
  if (upcoming.length > 0) {
    ...
    for (var i = 0; i < upcoming.length; i++) {
      var m = upcoming[i].m;
      lines.push("  " + upcoming[i].time + " " + (FLAGS[m.h]||"🏳️") + (CN[m.h]||m.h) + " vs " + (FLAGS[m.a]||"🏳️") + (CN[m.a]||m.a) + " (" + upcoming[i].mins + "min) [" + m.g + "组]");
    }
    lines.push("");
  }
  ...
  if (live.length === 0 && upcoming.length === 0 && finished.length === 0) {
    ...
    for (var i = 0; i < MATCHES.length; i++) {
      ...
      if (bj > now) {
        var hh = String(bj.getHours()).padStart(2, "0");
        var mm = String(bj.getMinutes()).padStart(2, "0");
        lines.push("\n📅 下一场: " + (bj.getMonth()+1) + "/" + bj.getDate() + " " + hh + ":" + mm);
        lines.push("  " + (FLAGS[MATCHES[i].h]||"🏳️") + (CN[MATCHES[i].h]||MATCHES[i].h) + " vs " + (FLAGS[MATCHES[i].a]||"🏳️") + (CN[MATCHES[i].a]||MATCHES[i].a));
        break;
      }
    }
  }
